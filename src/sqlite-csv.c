 ...
 * ** the data= parameter.
 * **
 * ** If the columns=N parameter is supplied, then the CSV file is assumed to
 * have
 ...
 * **
 * ** Some extra debugging features (used for testing virtual tables) are
 * available
 ...
 * */
static int csv_reader_open(CsvReader *p,          /* The reader to open */
 ...
 * */
static int csvtabConnect(sqlite3 *db, void *pAux, int argc,
  ...
  CsvReader
      sRdr; /* A CSV file reader used to store an error
                                            ** message and/or to count the
              *number of columns */
...
#define CSV_SCHEMA (azPValue[2])
  ...
  for (i = 3; i < argc; i++) {
    ...
    } else if ((zValue = csv_parameter("header", 6, z)) != 0) {
      int x;
      if (bHeader >= 0) {
 ...
 * */
static int csvtabCreate(sqlite3 *db, void *pAux, int argc,
                        const char *const *argv, sqlite3_vtab **ppVtab,
                        char **pzErr) {
 ...
 * */
static int
csvtabColumn(sqlite3_vtab_cursor *cur, /* The cursor */
 ...
 * ** the beginning.
 * */
static int csvtabFilter(sqlite3_vtab_cursor *pVtabCursor, int idxNum,
 ...
 * */
static int csvtabBestIndex(sqlite3_vtab *tab, sqlite3_index_info *pIdxInfo) {
  ...
  if ((((CsvTable *)tab)->tstFlags & CSVTEST_FIDX) != 0) {
     ...
     *             ** for testing virtual table logic but is not helpful to real
     * applications.
     *                 **
     *                     ** Any ==, LIKE, or GLOB constraint is marked as
     * usable by the virtual
     *                         ** table (even though it is not) and the cost of
     * running the virtual table
     *                             ** is reduced from 1 million to just 10.  The
     * constraints are *not* marked
     *                                 ** as omittable, however, so the query
     * planner should still generate a
     *                                     ** plan that gives a correct answer,
     * even if they plan is not optimal.
     *                                         */
    int i;
    int nConst = 0;
 ...
 * */
static int csvtabUpdate(sqlite3_vtab *p, int n, sqlite3_value **v,
                        sqlite3_int64 *x) {
...
__declspec(dllexport)
     ...
     * ** connection.
     * */
    int sqlite3_sqlitecsvlib_init(sqlite3 *db, char **pzErrMsg,
